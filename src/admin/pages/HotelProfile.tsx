...
export function HotelProfilePage({ hid }: { hid: string }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      fb.success(tr('Hotel profile saved'));
    } catch (e) {
  ...
  const togglePublish = async () => {
    ...
    const ok = await fb.confirm({
      title: next ? tr('Publish the guest site?') : tr('Take the guest site offline?'),
      message: next ? tr('Guests scanning QR codes will be able to use the site.') : tr('Guests will see “hotel not available”. Staff can still preview.'),
      confirmLabel: next ? tr('Publish') : tr('Unpublish'),
      danger: !next,
    });
    if (!ok) return;
    try {
      ...
      await hotel.refetch();
      fb.success(next ? tr('Guest site is live') : tr('Guest site is offline'));
    } catch (e) {
      fb.error(errorMessage(e));
    }
  };
  ...
  return (
    <>
      <PageHeader
        title={tr('Hotel profile')}
        description={tr('Names, contact details, languages, currency and VAT used across the guest site and requests.')}
        actions={
          <>
            <Badge tone={hotel.data.is_published ? 'success' : 'warning'}>{hotel.data.is_published ? tr('Guest site live') : tr('Guest site offline')}</Badge>
            <Button variant="secondary" size="sm" className="rounded-lg" onClick={togglePublish} loading={publication.isPending}>
              <Globe2 className="h-4 w-4" aria-hidden="true" /> {hotel.data.is_published ? tr('Unpublish') : tr('Publish')}
            </Button>
            <Button size="sm" className="rounded-lg" onClick={save} loading={saveProfile.isPending || saveSettings.isPending}>{tr('Save changes')}</Button>
          </>
        }
      />
      <div className="grid gap-4 xl:grid-cols-2">
        <Card title={tr('Identity')}>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={tr('Name (English)')} htmlFor="p-name-en" required error={err('name_en')}>
              <TextInput id="p-name-en" value={p.name_en} onChange={(e) => set('name_en', e.target.value)} className={c} />
            </Field>
            <Field label={tr('Name (Arabic)')} htmlFor="p-name-ar" required error={err('name_ar')}>
              <TextInput id="p-name-ar" dir="rtl" value={p.name_ar} onChange={(e) => set('name_ar', e.target.value)} className={c} />
            </Field>
            <Field label={tr('Web address (slug)')} htmlFor="p-slug" error={err('slug')} hint={me.data?.user?.global ? tr('Changing it breaks printed QR codes.') : tr('Only a super admin can change this.')}>
              <TextInput id="p-slug" value={p.slug} disabled={!me.data?.user?.global} onChange={(e) => set('slug', e.target.value.toLowerCase())} className={c} />
            </Field>
            <Field label={tr('Star rating')} htmlFor="p-stars">
            ...
            <Field label={tr('Description (Arabic)')} htmlFor="p-desc-ar">
        ...
        <Card title={tr('Contact & location')}>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={tr('Address (English)')} htmlFor="p-addr-en">
              <TextArea id="p-addr-en" rows={2} value={p.address_en} onChange={(e) => set('address_en', e.target.value)} className="rounded-lg text-sm" />
            </Field>
            <Field label={tr('Address (Arabic)')} htmlFor="p-addr-ar">
            ...
            <Field label={tr('City (English)')} htmlFor="p-city-en">
              <TextInput id="p-city-en" value={p.city_en} onChange={(e) => set('city_en', e.target.value)} className={c} />
            </Field>
            <Field label={tr('City (Arabic)')} htmlFor="p-city-ar">
              <TextInput id="p-city-ar" dir="rtl" value={p.city_ar} onChange={(e) => set('city_ar', e.target.value)} className={c} />
            </Field>
            <Field label={tr('Main phone')} htmlFor="p-phone" error={err('phone')}>
              <TextInput id="p-phone" type="tel" dir="ltr" value={p.phone} onChange={(e) => set('phone', e.target.value)} className={c} />
            </Field>
            <Field label={tr('Email')} htmlFor="p-email" error={err('email')}>
              <TextInput id="p-email" type="email" value={p.email} onChange={(e) => set('email', e.target.value)} className={c} />
            </Field>
            <Field label={tr('Website')} htmlFor="p-web" error={err('website')}>
              <TextInput id="p-web" type="url" dir="ltr" value={p.website} onChange={(e) => set('website', e.target.value)} className={c} />
            </Field>
            <Field label={tr('Map link')} htmlFor="p-map" error={err('map_url')} hint={tr('Google Maps share link used for “Directions”.')}>
              <TextInput id="p-map" type="url" dir="ltr" value={p.map_url} onChange={(e) => set('map_url', e.target.value)} className={c} />
            </Field>
            {SOCIALS.map((k) => (
              <Field key={k} label={k === 'x' ? tr('X (Twitter)') : k[0].toUpperCase() + k.slice(1)} htmlFor={`p-soc-${k}`} error={err(`social.${k}`)}>
                <TextInput id={`p-soc-${k}`} type="url" dir="ltr" placeholder={tr('https://')} value={p.social?.[k] ?? ''} onChange={(e) => set('social', { ...p.social, [k]: e.target.value })} className={c} />
              </Field>
            ))}
          </div>
        </Card>
        <Card title={tr('Languages, currency & VAT')}>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label={tr('Guest languages')} htmlFor="p-langs">
              <Select id="p-langs" value={p.language_mode} onChange={(e) => set('language_mode', e.target.value as HotelProfile['language_mode'])} className={c}>
                <option value="both">{tr('Arabic + English')}</option>
                <option value="ar">{tr('Arabic only')}</option>
                <option value="en">{tr('English only')}</option>
              </Select>
            </Field>
            <Field label={tr('Default language')} htmlFor="p-deflang">
            ...
            <Field label={tr('Currency (ISO code)')} htmlFor="p-cur" error={err('currency')}>
              <TextInput id="p-cur" maxLength={3} value={p.currency} onChange={(e) => set('currency', e.target.value.toUpperCase())} className={c} />
            </Field>
            <Field label={tr('Time zone')} htmlFor="p-tz" error={err('timezone')} hint={tr('Used for opening hours and daily figures.')}>
              <TextInput id="p-tz" value={p.timezone} onChange={(e) => set('timezone', e.target.value)} className={c} />
            </Field>
            <Field label={tr('VAT rate (%)')} htmlFor="p-vat" error={err('vat_rate')}>
            ...
            <div className="pt-6">
              <Toggle label={tr('Menu prices include VAT')} description={tr('Items can override this individually.')} checked={p.prices_include_vat} onChange={(v) => set('prices_include_vat', v)} />
            </div>
          </div>
        </Card>
        <Card title={tr('Guest settings')}>
          <div className="space-y-4">
            <Toggle label={tr('External visitors can use the site')} description={tr('Off = only in-house guests (room number required).')} checked={s.external_guests_enabled} onChange={(v) => setS({ ...s, external_guests_enabled: v })} />
            <Toggle label={tr('Phone number required')} checked={s.require_phone} onChange={(v) => setS({ ...s, require_phone: v })} />
            <Toggle label={tr('Guest reviews enabled')} description={tr('Reviews are published only after approval.')} checked={s.reviews_enabled} onChange={(v) => setS({ ...s, reviews_enabled: v })} />
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label={tr('WhatsApp fallback department')} htmlFor="s-fallback" hint={tr('Used when a department has no WhatsApp number.')}>
                <Select id="s-fallback" value={s.fallback_department ?? ''} onChange={(e) => setS({ ...s, fallback_department: (e.target.value || null) as HotelSettings['fallback_department'] })} className={c}>
                  <option value="">{tr('None (admin queue only)')}</option>
                  {DEPARTMENTS.map((d) => (
              ...
              <Field label={tr('Emergency phone')} htmlFor="s-emerg" error={errors.emergency_phone} hint={tr('Used by the Emergency quick action.')}>
