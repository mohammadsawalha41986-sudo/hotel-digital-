...
export function LaundryAdmin({ hid }: { hid: string }) {
  ...
  return (
    <>
      <PageHeader
        title="Laundry"
        description="Categories (gentlemen, ladies, traditional wear…) each contain garments with wash, dry-clean and press prices. Leave a price empty when that service is not offered."
      />
      <div className="grid gap-4 lg:grid-cols-[minmax(0,24rem)_minmax(0,1fr)]">
        <EntityManager hid={hid} entity="laundry_categories" compact onOpen={setCat} selectedId={cat?.id} title="Categories" />
        {cat ? (
          <EntityManager hid={hid} entity="laundry_items" parentId={cat.id} currency={currency} title={`Garments · ${String(cat.name_en)}`} />
        ) : (
          <p className="rounded-2xl border border-dashed border-black/15 p-6 text-sm text-zinc-500">Add a category to start.</p>
        )}
      </div>
      <div className="mt-6">
        <EntityManager hid={hid} entity="laundry_packages" currency={currency} title="Packages" />
      </div>
    </>
  );
}
