...
export function OrdersOverview({ hid, from, currency }: { hid: string; from: string; currency: string }) {
  ...
  if (q.error) return <ErrorState title={tr('Could not load order figures')} description={errorMessage(q.error)} onRetry={() => q.refetch()} />;
  const d = q.data!;
  const s = d.summary;
  return (
    <section aria-labelledby="orders-heading" className="space-y-4">
      <div className="flex items-baseline justify-between">
        <h2 id="orders-heading" className="text-lg font-semibold">{tr('Orders')}</h2>
        <Link to={`/admin/h/${hid}/orders`} className="text-sm font-medium text-zinc-600 hover:text-zinc-900">{tr('All orders →')}</Link>
      </div>
      <KpiGrid>
        <Kpi label={tr('Orders today')} value={s.orders_today} />
        <Kpi label={tr('New')} value={s.new} />
        <Kpi label={tr('Pending')} value={s.pending} hint={tr('{0} in progress', { 0: s.in_progress })} />
        <Kpi label={tr('Completed')} value={s.completed} />
        <Kpi label={tr('Cancelled / declined')} value={s.cancelled} />
        <Kpi label={tr('Order value (completed)')} value={major(s.order_value, currency)} />
        <Kpi label={tr('Average order value')} value={major(s.average_order_value, currency)} />
        {d.finance && <Kpi label={tr('Commission (this period)')} value={money(d.finance.commission_minor, currency)} hint={tr('{0} eligible orders', { 0: d.finance.eligible_orders })} />}
      </KpiGrid>
            ...
            {d.recent.map((o) => (
              <li key={o.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
                <span className="min-w-0">
                  ...
                  <span className="block text-xs text-zinc-500">{typeLabel(o.order_type)} · {o.room ? tr('Room {0}', { 0: o.room }) : tr('Visitor')} · {dateTime(o.created_at)}</span>
                </span>
                <span className="flex items-center gap-2">
                  <OrderStatus status={o.status} />
                  {d.finance_access === 'FULL' && <FinancialStatus status={o.financial_status} />}
                  <span className="w-20 text-end tabular-nums">{major(o.total, o.currency)}</span>
                </span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-sm text-zinc-500">{tr('No orders in this period.')}</p>
        )}
      </Card>
    </section>
  );
