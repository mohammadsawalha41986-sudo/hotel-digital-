...
export function PublishDialog({ hid, open, onClose }: { hid: string; open: boolean; onClose: () => void }) {
  ...
  const m = useMutation({
    ...
    onSuccess: (r) => {
      fb.success(tr('Published version {0} — guests now see these changes', { 0: r.publication.version }));
      setNote('');
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={tr('Publish updates')}
      description={tr('Content, prices, images, branding and the website layout go live together as a new version. Availability and routing changes already apply immediately.')}
      size="md"
      footer={
        <div className="flex items-center justify-between gap-3">
          <Link to={`/admin/h/${hid}/publishing`} onClick={onClose} className="inline-flex items-center gap-1.5 text-sm text-zinc-600 hover:text-zinc-900">
            <History className="h-4 w-4" aria-hidden="true" />{' '}{tr('Publishing history')}</Link>
          <Button loading={m.isPending} disabled={!s?.has_unpublished_changes} onClick={() => m.mutate()}>
            <UploadCloud className="h-4 w-4" aria-hidden="true" />{' '}{tr('Publish now')}</Button>
        </div>
      }
    >
      {state.isLoading ? (
        ...
        <div className="space-y-4">
          <p className="text-sm text-zinc-600">
            {s.latest ? tr('Live: version {0}, published {1}.', { 0: s.latest.version, 1: when(s.latest.published_at) }) : tr('Nothing has been published yet.')}{' '}
            {s.has_unpublished_changes ? '' : tr('Everything is published.')}
          </p>
          {s.changes.length > 0 && (
            <div>
              <p className="mb-2 text-sm font-medium">{tr('Changes since the last publish ({0}', { 0: s.changes.length })}{s.changes.length === 100 ? '+' : ''})</p>
          ...
          {s.has_unpublished_changes && (
            <Field label={tr('Note (optional)')} htmlFor="pub-note" hint={tr('Shown in the publishing history, e.g. “Ramadan menu”.')}>
...
export function PublishControls({ hid }: { hid: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {pending ? <Badge tone="warning">{tr('Unpublished changes')}</Badge> : state.data ? <Badge tone="success">{tr('All changes live')}</Badge> : null}
      {slug && (
        <a href={`/h/${slug}?preview=1`} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 items-center gap-1.5 rounded-lg px-3 text-sm font-medium text-zinc-700 ring-1 ring-black/10 hover:bg-zinc-50">
          <Eye className="h-4 w-4" aria-hidden="true" />{' '}{tr('Preview')}</a>
      )}
      <Button size="sm" className="rounded-lg" onClick={() => setOpen(true)}>
        <UploadCloud className="h-4 w-4" aria-hidden="true" />{' '}{tr('Publish updates')}{pending && state.data!.changes.length > 0 && <span className="ms-1 rounded-full bg-white/20 px-1.5 text-xs tabular-nums">{state.data!.changes.length}</span>}
      </Button>
