...
export function ThemePreview({ branding, nameEn, nameAr, device }: { branding: Branding; nameEn: string; nameAr: string; device: 'mobile' | 'desktop' }) {
  ...
  return (
    <div className={cx('mx-auto overflow-hidden rounded-[1.6rem] border border-black/10 shadow-sm', mobile ? 'w-[340px]' : 'w-full')} style={{ ...vars, background: 'var(--c-bg)', color: 'var(--c-text)', fontFamily: 'var(--font-body)', fontSize: 'var(--font-size-base)' }} aria-label={`${device} preview`}>
      <header className="flex items-center justify-between px-4 py-3" style={{ background: 'var(--c-surface)', borderBottom: '1px solid var(--c-line)' }}>
        ...
        <span className="rounded-full px-2.5 py-1 text-[0.7rem] font-semibold" style={{ background: 'var(--c-light)', color: 'var(--c-muted)' }}>EN · ع</span>
      </header>
      <section className={cx('relative px-5', mobile ? 'py-8' : 'py-12')} style={{ background: 'linear-gradient(135deg, var(--c-secondary), var(--c-dark))', color: 'var(--c-secondary-ink)' }}>
        ...
        <p className="text-[0.65rem] tracking-[0.2em] uppercase" style={{ color: 'var(--c-accent)' }}>Welcome</p>
        ...
        <button type="button" tabIndex={-1} className="mt-5 inline-flex h-10 items-center rounded-full px-5 text-sm font-semibold" style={{ background: 'var(--c-cta)', color: 'var(--c-cta-ink)' }}>
          Order to your room
        </button>
      </section>
        ...
        {['Lobby Café', 'Wellness'].map((t, i) => (
          <article key={t} className="overflow-hidden rounded-2xl" style={{ background: 'var(--c-surface)', border: '1px solid var(--c-line)' }}>
            ...
            <div className="p-3">
              ...
              <p className="text-xs" style={{ color: 'var(--c-muted)' }}>Secondary text · open until 23:00</p>
              <div className="mt-2 flex items-center justify-between">
                <span className="text-sm font-semibold tabular-nums">SAR 45</span>
                <button type="button" tabIndex={-1} className="h-8 rounded-full px-3 text-xs font-semibold transition" style={{ background: i ? 'var(--c-cta-hover)' : 'var(--c-cta)', color: 'var(--c-cta-ink)' }}>
                  {i ? 'Hover state' : 'Add'}
                </button>
              </div>
            </div>
      ...
      <footer className="px-5 py-4 text-xs" style={{ background: 'var(--c-dark)', color: 'var(--c-dark-ink)' }}>
        <span style={{ opacity: 0.75 }}>Reception · +966 11 000 0000</span>
      </footer>
    </div>
  );
}
