...
import { reviewInputSchema } from '../../shared/hotel';
...
publicRoutes.post('/hotels/:slug/requests/:reference/cancel', async (c) => {
  ...
  const r = await one<{ id: string; status: string }>(
    `UPDATE requests SET status = 'CANCELLED', cancelled_at = now(), updated_at = now()
      WHERE hotel_id = $1 AND reference = $2 AND guest_token_hash = $3 AND status = 'NEW'
      RETURNING id, status`,
    [row.id, c.req.param('reference'), guestTokenHash(c)]
  );
  if (!r) throw new HttpError(409, 'not_cancellable', 'This request has already been accepted by the team and can no longer be cancelled here.');
  await q(`INSERT INTO request_events (request_id, hotel_id, from_status, to_status, note, is_internal) VALUES ($1,$2,'NEW','CANCELLED','Cancelled by guest',false)`, [r.id, row.id]);
...
publicRoutes.post('/hotels/:slug/reviews', async (c) => {
  ...
  await q(`INSERT INTO reviews (hotel_id, guest_name, room, rating, title, body, lang) VALUES ($1,$2,$3,$4,$5,$6,$7)`, [
    row.id,
    v.guest_name,
    v.room,
    v.rating,
