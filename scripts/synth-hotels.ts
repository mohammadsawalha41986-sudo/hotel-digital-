 ...
 * (4 outlets × 5 categories × 8 items, room/hotel services, spa, laundry,
 * offers), its own departments, staff accounts, a commission agreement and a
 * publication. Order history (`--days` × `--orders` per day, clustered around
 * breakfast and dinner) is bulk-inserted in SQL so indexes and plans are
 * exercised at production-like table sizes. All names and phone numbers are
 * obviously fake; no real guest data is used.
 */
import { parseArgs } from 'node:util';
import { brandingSchema, settingsSchema } from '../shared/hotel';
import { defaultSiteConfig } from '../shared/defaults';
import { migrate, one, pool, q, tx } from '../server/db';
import { createEntity } from '../server/repos/entities';
import { ensureDepartments } from '../server/repos/hotels';
import { publishHotel } from '../server/services/publish';
import { upsertUser, DEMO_PASSWORD } from '../server/seed/demo';
...
const HOTELS = Number(values.hotels);
const DAYS = Number(values.days);
const PER_DAY = Number(values.orders);

if (process.env.NODE_ENV === 'production') throw new Error('Refusing to generate synthetic data in production');
if (!/_load|_staging|_test|localhost/.test(process.env.DATABASE_URL ?? '')) console.warn('Warning: DATABASE_URL does not look like a load/staging database');
...
async function main() {
  ...
  for (let n = 1; n <= HOTELS; n++) {
    ...
    orders += added;
    console.log(`${h.slug}: ${h.created ? 'created' : 'exists'}; +${added} historical orders`);
  }
  await pool.query('ANALYZE');
  const counts = await one(`SELECT (SELECT COUNT(*) FROM hotels) AS hotels, (SELECT COUNT(*) FROM requests) AS requests, (SELECT COUNT(*) FROM order_lines) AS lines, (SELECT COUNT(*) FROM guests) AS guests, pg_size_pretty(pg_database_size(current_database())) AS size`);
